...
def add_padding(value, max_length):
    new_value = value
    for _ in range(len(value), max_length):
        new_value += EOS
    return new_value

def prepare_tokens(tokens, max_length, error_rate=0.3):
    encoder_tokens = []
    decoder_tokens = []
    target_tokens = []
    for token in tokens:
        encoder_token = add_speling_erors(token, error_rate=error_rate)
        encoder_tokens.append(add_padding(encoder_token, max_length))
        decoder_token = SOS + token
        decoder_tokens.append(add_padding(decoder_token, max_length)) 
        target_token = decoder_token[1:]
        target_tokens.append(add_padding(target_token, max_length))
    return encoder_tokens, decoder_tokens, target_tokens


def get_batch_generator(tokens, maxlen, ctable, batch_size=128, reverse=False):
...
def generate_data(encoder_inputs, decoder_inputs, targets):
    generatad_data = []
    for i in range(0, len(encoder_inputs)):
        generatad_data.append([encoder_inputs[i], decoder_inputs[i], targets[i]])
    return generatad_data
...
def decode_sequences(inputs, targets, input_ctable, target_ctable,
    ...
    input_sequences = get_batch_generator(input_tokens, maxlen, input_ctable,
                                          nb_examples, reverse)
